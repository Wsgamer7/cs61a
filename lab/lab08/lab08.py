...
def multiply_lnks(lst_of_link):
    ...
    if is_recursive:
        return Link(result, multiply_lnks(new_lst))
    else:
        return Link(result)
def flip_two(lnk):
    if lnk.rest != Link.empty and lnk.rest.rest != Link.empty:
        temp = lnk.first
        lnk.first = lnk.rest.first
        lnk.rest.first = temp
        flip_two(lnk.rest.rest)
    elif lnk.rest != Link.empty and lnk.rest.rest == Link.empty:
        temp = lnk.first
        lnk.first = lnk.rest.first
        lnk.rest.first = temp
...
def reverse_other(t):
    ...
    if not t.is_leaf():
        reversed_lst = []
        for b in t.branches:
            reversed_lst = [b.label] + reversed_lst
        i = 0
        for b in t.branches:
            b.label = reversed_lst[i]
            i += 1
            if not b.is_leaf():
