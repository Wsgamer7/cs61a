...
class CheckingAccount(Account):
    ...
    "*** YOUR CODE HERE ***"
    def deposit_check(self, a_check):
        if a_check.deposited:
            print('The police have been notified.')
        elif a_check.payable_to != self.holder:
            print('The police have been notified.')
        else:
            now_balance= self.deposit(a_check.amount)
            a_check.deposited = True
            return now_balance
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            possibilities.append((str(skel_char) + match, 0 + cost))
        # Insert
        edits = "".join("+[" + str(code_char) + "]")
        match, cost = helper_align(skeleton_idx, code_idx + 1)
        possibilities.append((edits + match, 1 + cost))
        # Delete
        edits = "".join("-[" + str(skel_char) + "]")
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda x, y : Link(y, x) if y else y, Link.empty)
